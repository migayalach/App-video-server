...
export class DownloadService {
  ...
  async downloadVideo(url: string, quality: string, format: string) {
    ...
    exec(infoCommand, (err, stdout, stderr) => {
      // If an error occurs while getting video info, throw an HTTP exception
      if (err || stderr) {
      ...
      const videoTitle = stdout.trim(); // Remove extra spaces

      // Ensure the file name is safe (no invalid characters like slashes)
